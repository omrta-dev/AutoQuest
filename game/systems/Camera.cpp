...
aik::Camera::Camera(): _viewportX(0), _viewportY(0), _windowX(1920), _windowY(1080), _lastX(0), _lastY(0), _aspect(1), _nearClip(.001f), _farClip(1000.0f), _cameraHeading(0), _cameraPitch(0), _grab(false)
{
...
void aik::Camera::reset()
{
	_cameraUp = glm::vec3(0.0f, 1.0f, 0.0f);
}

void aik::Camera::update()
{
...
void aik::Camera::changePitch(float degrees)
{
...
void aik::Camera::changeHeading(float degrees)
{
...
void aik::Camera::processMouseMovement(sf::RenderWindow& window)
{
	...
	{
		float xOffset = (_windowX / 2) - mousePos.x;
		float yOffset = (_windowY / 2) - mousePos.y;
...
void aik::Camera::setMode(CameraType type)
{
	_cameraType = type;
	_cameraUp = glm::vec3(0.0f, 1.0f, 0.0f);
}

void aik::Camera::setPosition(glm::vec3 pos)
{
	_cameraPos = pos;
}

void aik::Camera::setLookAt(glm::vec3 pos)
{
	_cameraLookAt = pos;
}

void aik::Camera::setFOV(double fov)
{
	_fov = fov;
}

void aik::Camera::setViewport(int locX, int locY, int width, int height)
{
	_viewportX = locX;
	_viewportY = locY;
	_windowX = width;
...
void aik::Camera::setClipping(double nearClipDistance, double farClipDistance)
{
	_nearClip = nearClipDistance;
	_farClip = farClipDistance;
}

void aik::Camera::processMouseButtons()
{
    _moveCamera = sf::Mouse::isButtonPressed(sf::Mouse::Button::Left);
    _grab = sf::Mouse::isButtonPressed(sf::Mouse::Button::Right);
}

aik::CameraType aik::Camera::getMode()
{
	return _cameraType;
}

void aik::Camera::getViewPort(int& locX, int& locY, int& width, int& height)
{
	locX = _viewportX;
	locY = _viewportY;
	width = _windowX;
...
float aik::Camera::getDirection() const
{
	return static_cast<float>(acos(glm::dot(_oldCameraDirection.x, _cameraDirection.x)));
}

void aik::Camera::processMovement(CameraDirection direction)
{
	if (_cameraType == FREE)
	{
		switch (direction)
